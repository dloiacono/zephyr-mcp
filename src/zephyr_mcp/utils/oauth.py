...
logger = logging.getLogger("mcp-zephyr.oauth")
...
KEYRING_SERVICE_NAME = "mcp-zephyr-oauth"
...
class OAuthConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.cloud_id and self.base_url:
            if is_atlassian_cloud_url(self.base_url):
                ...
                raise ValueError("OAuthConfig cannot have both cloud_id and base_url set. Use cloud_id for Cloud or base_url for Data Center.")
        ...
        try:
            ...
            if "refresh_token" not in token_data:
                if self.is_data_center:
                    logger.warning("No refresh_token in DC response — token cannot be refreshed.")
                else:
                    logger.error("Refresh token not found in response. Ensure 'offline_access' scope is included.")
                    return False
            ...
            logger.error(f"Failed to decode JSON response from token endpoint: {e}", exc_info=True)
            return False
    ...
    def _save_tokens(self) -> None:
        """Save the tokens securely using keyring."""
        try:
    ...
    def _save_tokens_to_file(self, token_data: dict | None = None) -> None:
        ...
        try:
            token_dir = Path.home() / ".zephyr-mcp"
    ...
    def _load_tokens_from_file(client_id: str) -> dict[str, Any]:
        ...
        token_path = Path.home() / ".zephyr-mcp" / f"oauth-{client_id}.json"
    ...
    @classmethod
    def from_env(cls, service_url: str | None = None) -> Optional["OAuthConfig"]:
...
class BYOAccessTokenOAuthConfig:
    ...
    @classmethod
    def from_env(cls, service_url: str | None = None) -> Optional["BYOAccessTokenOAuthConfig"]:
...
def get_oauth_config_from_env(service_url: str | None = None) -> OAuthConfig | BYOAccessTokenOAuthConfig | None:
    """Get the appropriate OAuth configuration from environment variables."""
    return BYOAccessTokenOAuthConfig.from_env(service_url=service_url) or OAuthConfig.from_env(service_url=service_url)


def configure_oauth_session(session: requests.Session, oauth_config: OAuthConfig | BYOAccessTokenOAuthConfig) -> bool:
    ...
    if not oauth_config.access_token and not oauth_config.refresh_token:
        logger.warning("No access_token or refresh_token available. Cannot configure OAuth session.")
        return False
    ...
    if isinstance(oauth_config, BYOAccessTokenOAuthConfig):
        logger.error("BYO access token configuration provided as empty string.")
        return False
